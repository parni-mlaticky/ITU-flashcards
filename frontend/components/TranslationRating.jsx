...
const translationRating = ({translation, article}) => {
    ...
    const sendNewRating = async (rating) => {
        ...
        const sendRating = async (rating) => {
            await axios.post(`/translations/${translation.translation.id}/rating`, {rating: rating, userId: userId});
    ...
    const unsetRating = async () => {
        ...
        const unsetRating = async () => {
            await axios.delete(`/translations/${translation.translation.id}/rating`, {data:{userId: userId}});
        }
        unsetRating();
        setRating(null);
        console.log("Unrated");
    ...
    useEffect(() => {
        const fetchRating = async () => {
            const userId = await AsyncStorage.getItem("user");
            const query = `/articles/${article.id}/translation/rate/${translation.author_id}`
    ...
    return (
        ...
        <Box safeAreaBottom={20} alignItems={"center"}>
            <Text fontSize={30}>Rate this translation</Text>
            <HStack>
                <Text style={{verticalAlign:"middle"}} marginTop={4} marginLeft={5} marginRight={5}>Poor</Text>
                ...
                <Text style={{verticalAlign:"middle"}} marginTop={4} marginLeft={5} marginRight={5}>Excellent</Text>
            </HStack>
        </Box>
        </>
    );
